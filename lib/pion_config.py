...
class PionConfig():
    ...
    def slurp_config_file(self, filename):
        # read pion.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is pion.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from pion.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Pion defaults...
        default_port = 9953 if (network == 'mainnet') else 19953

        # use default port for network if not specified in pion.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from pion.conf
