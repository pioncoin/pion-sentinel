...
class GovernanceObject(BaseModel):
    ...
    # sync piond gobject list with our local relational DB backend
    @classmethod
    def sync(self, piond):
        golist = piond.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_piond(piond, item)
    ...
    @classmethod
    def import_gobject_from_piond(self, piond, rec):
        import decimal
        import pionlib
        import inflection
        ...
        # shim/piond conversion
        object_hex = pionlib.SHIM_deserialise_from_piond(object_hex)
        objects = pionlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from piond...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from piond, with every run
        ...
        # get/create, then sync payment amounts, etc. from piond - Piond is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(piond)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from piond! %s" % e)
            govobj.vote_delete(piond)
            return (govobj, None)
    ...
    def vote_delete(self, piond):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(piond, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, piond, signal, outcome):
        import pionlib
        ...
        output = piond.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = pionlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(piond, signal)

    def sync_network_vote(self, piond, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = piond.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = PIOND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 pion addr, non-multisig
            if not pionlib.is_valid_pion_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Pion address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (pionlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/PionDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import pionlib
        obj_data = pionlib.SHIM_serialise_for_piond(self.serialise())
    ...
    def prepare(self, piond):
        try:
            object_hash = piond.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = PIOND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not pionlib.is_valid_pion_address(addr, config.network):
    ...
    def hash(self):
        import pionlib
        return pionlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = PIOND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, piond):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - piond.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, piond):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - piond.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, piond):
        now = int(time.time())
        return (self.created_at < (now - piond.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, piond):
        if self.is_expired(piond):
            return False

        return True

    def is_deletable(self, piond):
        if self.is_expired(piond):
            return True
