...
class PionDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some piond methods
    ...
    @classmethod
    def from_pion_conf(self, pion_dot_conf):
        from pion_config import PionConfig
        config_text = PionConfig.slurp_config_file(pion_dot_conf)
        creds = PionConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = pionlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = pionlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = pionlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_piond_proto_version_with_sentinel_ping)
