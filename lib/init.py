...
def has_pion_conf():
    ...
    valid_pion_conf = False

    # ensure pion_conf exists & readable
    #
    # if not, print a message stating that Pion Core must be installed and
    # configured, including JSONRPC access in pion.conf
    try:
        f = io.open(config.pion_conf)
        valid_pion_conf = True
    except IOError as e:
        print(e)

    return valid_pion_conf
...
def main():
    ...
    if not has_pion_conf():
        print("PionCore must be installed and configured, including JSONRPC access in pion.conf")
        sys.exit(1)
