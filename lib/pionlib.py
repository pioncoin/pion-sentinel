...
def is_valid_pion_address(address, network='mainnet'):
    ...
    pion_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Pion address
        return False

    if (address_version != pion_version):
        return False
...
# shims 'til we can fix the piond side
def SHIM_serialise_for_piond(sentinel_hex):
    from models import PIOND_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = PIOND_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in piond
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # piond expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    piond_hex = serialise(obj)
    return piond_hex


# shims 'til we can fix the piond side
def SHIM_deserialise_from_piond(piond_hex):
    from models import PIOND_GOVOBJ_TYPES

    # unpack
    obj = deserialise(piond_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('pion.conf')
